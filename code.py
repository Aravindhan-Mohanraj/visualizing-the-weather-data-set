...
def line_chart(df,period,col):
    ...
    month_wise = df.groupby(["month"])[col].mean()
    xlabels = []
    for i in range(1, 13):
        xlabels.append(calendar.month_name[i])
    print(xlabels)

    print(month_wise)
    plt.plot(month_wise)
    plt.xticks(np.arange(1, 13), xlabels, rotation=90)
    plt.show()
...
line_chart(weather_df,'01/01/2012','Temp (C)')
